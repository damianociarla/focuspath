...
export function generateHtmlReport(report: FocusReport): string {
  ...
  const stepRows = report.steps.map((step) => `<tr><th scope="row">${step.index}</th><td><code>${escapeHtml(step.selector)}</code></td><td>${escapeHtml(step.role ?? step.tagName)}</td><td>${escapeHtml(step.accessibleName || "—")}</td><td>${escapeHtml(step.focusIndicator.outline)}</td></tr>`).join("");
