...
FOCUSPATH_PROFILE="${AWS_PROFILE:-portfolio-bootstrap}"
...
FOCUSPATH_AWS=(aws --profile "${FOCUSPATH_PROFILE}" --region "${FOCUSPATH_REGION}")
...
if [[ -f "${FOCUSPATH_TOKEN_FILE}" ]]; then
...
"${FOCUSPATH_AWS[@]}" cloudformation deploy \
  ...
  --parameter-overrides "ImageIdentifier=${FOCUSPATH_IMAGE}" "AllowedOrigin=${FOCUSPATH_ORIGIN}" "OriginVerifyToken=${FOCUSPATH_ORIGIN_TOKEN}" "BudgetAlertEmail=${FOCUSPATH_BUDGET_EMAIL}"
...
printf 'Enroll the distribution in the CloudFront Free flat-rate plan, then set VITE_API_URL to the protected URL.\n'
